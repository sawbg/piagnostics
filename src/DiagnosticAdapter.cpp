...
namespace piagnostics {
	...
	std::string DiagnosticAdapter::BarometricPressure() {
		...
		return (lang == English ? ("Baro Press: " + std::to_string(val) + " " + "PSI")
				: ("Luftdruck: " + std::to_string(val) + " " + "kPa"));
	}
	...
	std::string DiagnosticAdapter::CheckEngineLight() {
		...
		switch(lang) {
			case English:
				ret = std::string("Problem Light: ") + (on ? "On" : "Off");
				break;
			case German:
				ret = std::string("MIL: ") + (on ? "Ein" : "Aus");
				break;
		}
	...
	std::string DiagnosticAdapter::CoolantTemp() {
		...
		return (lang == English ? "Coolant: " : ("K" + ue + std::string("hlmittel: ")))
			+ std::to_string(val) + degree  + (units == Units::Imperial ? "F" : "C");
	}

	std::string DiagnosticAdapter::EngineLoad() {
		int val = to_int(FetchData(Pid::EngineLoad)) / 255. * 100;
		return (lang == English ? "Engine Load: " : "Motorlast: ") + std::to_string(val) + "%";
	}
	...
	std::vector<uint8_t> DiagnosticAdapter::FetchData(Pid pid) {
		...
		std::vector<uint8_t> resp = conn.Fetch(mode + ss.str() + std::to_string(bytes));
		// error code here
	...
	std::string DiagnosticAdapter::MilesSinceCodesCleared() {
		...
		return (lang == English ? ("DTC clr " + std::to_string(val) + "mi ago")
				: ("DTC gefrt v " + std::to_string(val) + "km"));
	}

	std::string DiagnosticAdapter::MinutesSinceStart() {
		int val = (to_int(FetchData(Pid::SecondsSinceStart)) + 30) / 60;
		return (lang == English ? ("Started " + std::to_string(val) + "m ago")
				: ("Gestartet vor " + std::to_string(val) + "m"));
	}
	...
	std::string DiagnosticAdapter::TimingAdvance() {
		...
		return (lang == English ? "Timing Adv.: " : (std::string("Fr")
				+ ue + "hz" + ue + "ndung: ")) + std::to_string(val) + degree;
	}
